...
class BalanceReportsGrossNotifiableModule(AbstractClassNotifiableModule):
    main_model = Report
    module_name = Modules.BALANCE_REPORTS_GROSS

    def get_specified_fields(self):
        return super().get_specified_fields()

    def get_fields(self):
        specified_fields = ('creation_date', 'report_name', 'status', 'month', 'year', 'report_type__initials')
    ...
    def post_save_signal_handler(sender, instance, **kwargs):
        ...
        if instance.status == 'T':
            ...
            kwargs["notifiable_module"] = BalanceReportsGrossNotifiableModule()
            kwargs["created"] = True
